...
# Register the launcher in whatever onCreate shape exists. This runs after the persistence patch.
launcher='authorizationLauncher=registerForActivityResult(new ActivityResultContracts.StartIntentSenderForResult(),result->{if(result.getResultCode()!=RESULT_OK){showMessage("Google authorization","Authorization did not complete. Result code: "+result.getResultCode());return;}Intent data=result.getData();if(data==null){showMessage("Google authorization","Google returned no authorization data.");return;}try{handleAuthorization(Identity.getAuthorizationClient(this).getAuthorizationResultFromIntent(data));}catch(ApiException e){showAuthError("Authorization result error",e);}});'
if 'authorizationLauncher=registerForActivityResult' not in s:
    m=re.search(r'@Override public void onCreate\(Bundle b\)\{super\.onCreate\(b\);',s)
    if m:
        pos=m.end()
        s=s[:pos]+launcher+s[pos:]
    else:
        raise SystemExit('Could not locate onCreate for OAuth launcher registration')

# Replace the old startIntentSenderForResult call.
...
# Normalize any remaining try/catch around a direct launcher invocation.
s=re.sub(r'try\s*\{\s*(authorizationLauncher\.launch\([^;]+\);)\s*\}\s*catch\s*\(\s*IntentSender\.SendIntentException\s+e\s*\)\s*\{[^}]*\}', r'\1', s, flags=re.S)

# Remove obsolete ActivityResult callback if present.
...
p.write_text(s)
print('Fixed OAuth launcher registration after persistence patch')